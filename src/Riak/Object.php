...
class Object {
	 ...
	 * @param string $key
	 *        	- An optional key. If not specified, then key
	 *        	is generated by server when store(...) is called.
	 */
	function __construct($client, $bucket, $key = NULL) {
		$this->client = $client;
		$this->bucket = $bucket;
		$this->key = $key;
		$this->jsonize = TRUE;
	 ...
	 * This is either application/json, or
	 ...
	 * 
	 * @param mixed $obj
	 *        	- Either a Riak\Object or a Riak\Link object.
	 * @param string $tag
	 *        	- Optional link tag. (default is bucket name,
	 *        	ignored if $obj is a Riak\Link object.)
	 * @return Riak\Object
	 */
	function addLink($obj, $tag = NULL) {
		if ($obj instanceof Riak\Link)
			$newlink = $obj;
		else
			$newlink = new Riak\Link ( $obj->bucket->name, $obj->key, $tag );
	...
	function removeLink($obj, $tag = NULL) {
		if ($obj instanceof Riak\Link)
			...
			$oldlink = new Riak\Link ( $obj->bucket->name, $obj->key, $tag );
	 ...
	 *        	- Must be one of 'int' or 'bin' - the
	 *        	only two index types supported by Riak
	 * @param
	 *        	string|int optional $explicitValue - If provided, uses this
	 ...
	 *        	- must be 'bin' or 'int'
	 * @param
	 *        	string|int optional $explicitValue
	 * @return $this
	 */
	 ...
	 *
	 * @param
	 *        	string optional $indexName
	 * @param
	 *        	string optional $indexType
	 *        	
	 * @return $this
	 */
	 ...
	 * 
	 * @param integer $w
	 *        	- W-value, wait for this many partitions to respond
	 *        	before returning to client.
	 * @param integer $dw
	 *        	- DW-value, wait for this many partitions to
	...
	function store($w = NULL, $dw = NULL) {
		...
		count ( $this->autoIndexes ) > 0 ? $this->meta ['x-rc-autoindex'] = json_encode ( $this->autoIndexes ) : $this->meta ['x-rc-autoindex'] = null;
		count ( $collisions ) > 0 ? $this->meta ['x-rc-autoindexcollision'] = json_encode ( $collisions ) : $this->meta ['x-rc-autoindexcollision'] = null;
	 ...
	 * 
	 * @param integer $r
	 *        	- R-Value, wait for this many partitions to respond
	 *        	before returning to client.
	 * @return $this
	 *
	 */
	 ...
	 * 
	 * @param integer $dw
	 *        	- DW-value. Wait until this many partitions have
	 ...
	 * Only for use by the Riak client
	 * library.
	 * 
	 * @return $this
	 */
	 ...
	 *        	- Sibling number.
	 * @param integer $r
	 *        	- R-Value. Wait until this many partitions
	 ...
	 * 
	 * @param integer $r
	 *        	- R-Value. Wait until this many partitions have
